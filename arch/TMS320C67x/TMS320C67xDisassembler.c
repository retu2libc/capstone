...
#include "TMS320C67xGenDisassemblerTables.inc"

#define GET_REGINFO_ENUM
#define GET_REGINFO_MC_DESC
#include "TMS320C67xGenRegisterInfo.inc"

static const unsigned GPRegsDecoderTable[] = {
	TMS320C67x_A0,  TMS320C67x_A1,  TMS320C67x_A2,  TMS320C67x_A3,
	TMS320C67x_A4,  TMS320C67x_A5,  TMS320C67x_A6,  TMS320C67x_A7,
	TMS320C67x_A8,  TMS320C67x_A9,  TMS320C67x_A10, TMS320C67x_A11,
	TMS320C67x_A12, TMS320C67x_A13, TMS320C67x_A14, TMS320C67x_A15,
	TMS320C67x_A16, TMS320C67x_A17, TMS320C67x_A18, TMS320C67x_A19,
	TMS320C67x_A20, TMS320C67x_A21, TMS320C67x_A22, TMS320C67x_A23,
	TMS320C67x_A24, TMS320C67x_A25, TMS320C67x_A26, TMS320C67x_A27,
	TMS320C67x_A28, TMS320C67x_A29, TMS320C67x_A30, TMS320C67x_A31
};
		...
		uint64_t Address, void *Decoder)
{
	...
	if((base >= TMS320C67X_REG_A0) && (base <= TMS320C67X_REG_A31))
		base = (base - TMS320C67X_REG_A0 + TMS320C67X_REG_B0);
	// base cannot be a B register, because it was ANDed above with 0x1f.
	// And the TMS320C67X_REG_B0 > 31
	...
	switch(mode) {
		...
		case 14:
		case 15:
			if((offset >= TMS320C67X_REG_A0) && (offset <= TMS320C67X_REG_A31))
				offset = (offset - TMS320C67X_REG_A0 + TMS320C67X_REG_B0);
		...
		uint64_t Address, void *Decoder)
{
	...
	if(Val & 1)
		basereg = TMS320C67X_REG_B15;
	else
		basereg = TMS320C67X_REG_B14;
		...
		uint64_t Address, void *Decoder)
{
	...
	switch(Val) {
		case 0:
		case 7:
			Inst->flat_insn->detail->tms320c67x.condition.reg = TMS320C67X_REG_INVALID;
			break;
		case 1:
			Inst->flat_insn->detail->tms320c67x.condition.reg = TMS320C67X_REG_B0;
			break;
		case 2:
			Inst->flat_insn->detail->tms320c67x.condition.reg = TMS320C67X_REG_B1;
			break;
		case 3:
			Inst->flat_insn->detail->tms320c67x.condition.reg = TMS320C67X_REG_B2;
			break;
		case 4:
			Inst->flat_insn->detail->tms320c67x.condition.reg = TMS320C67X_REG_A1;
			break;
		case 5:
			Inst->flat_insn->detail->tms320c67x.condition.reg = TMS320C67X_REG_A2;
			break;
		case 6:
			Inst->flat_insn->detail->tms320c67x.condition.reg = TMS320C67X_REG_A0;
			break;
		default:
			Inst->flat_insn->detail->tms320c67x.condition.reg = TMS320C67X_REG_INVALID;
		...
		uint64_t Address, void *Decoder)
{
	...
	switch(Val) {
		case 0:
			Inst->flat_insn->detail->tms320c67x.condition.zero = 0;
			break;
		case 1:
			Inst->flat_insn->detail->tms320c67x.condition.zero = 1;
			break;
		default:
			Inst->flat_insn->detail->tms320c67x.condition.zero = 0;
			ret = MCDisassembler_Fail;
		...
		uint64_t Address, void *Decoder)
{
	...
	if(Val == 1) {
		for(i = 0; i < Inst->size; i++) {
			...
			if(op->Kind == kRegister) {
				if((op->RegVal >= TMS320C67X_REG_A0) && (op->RegVal <= TMS320C67X_REG_A31))
					op->RegVal = (op->RegVal - TMS320C67X_REG_A0 + TMS320C67X_REG_B0);
				else if((op->RegVal >= TMS320C67X_REG_B0) && (op->RegVal <= TMS320C67X_REG_B31))
					op->RegVal = (op->RegVal - TMS320C67X_REG_B0 + TMS320C67X_REG_A0);
			}
		}
	}
	...
	switch(Val) {
		case 0:
			Inst->flat_insn->detail->tms320c67x.funit.side = 1;
			break;
		case 1:
			Inst->flat_insn->detail->tms320c67x.funit.side = 2;
			break;
		default:
			Inst->flat_insn->detail->tms320c67x.funit.side = 0;
			ret = MCDisassembler_Fail;
			break;
	}
		...
		uint64_t Address, void *Decoder)
{
	...
	switch(Val) {
		case 0:
			Inst->flat_insn->detail->tms320c67x.parallel = 0;
			break;
		case 1:
			Inst->flat_insn->detail->tms320c67x.parallel = 1;
			break;
		default:
			Inst->flat_insn->detail->tms320c67x.parallel = -1;
			ret = MCDisassembler_Fail;
		...
		uint64_t Address, void *Decoder)
{
	...
	switch(Val) {
		case 0:
			Inst->flat_insn->detail->tms320c67x.funit.crosspath = 0;
			break;
		case 1:
			Inst->flat_insn->detail->tms320c67x.funit.crosspath = 1;
			op = &Inst->Operands[0];
			if(op->Kind == kRegister) {
				if((op->RegVal >= TMS320C67X_REG_A0) && (op->RegVal <= TMS320C67X_REG_A31))
					op->RegVal = (op->RegVal - TMS320C67X_REG_A0 + TMS320C67X_REG_B0);
		...
		uint64_t Address, void *Decoder)
{
	...
	switch(Val) {
		case 0:
			...
			break;
		case 1:
			Inst->flat_insn->detail->tms320c67x.funit.crosspath = 2;
			op = &Inst->Operands[2];
			if(op->Kind == kRegister) {
				if((op->RegVal >= TMS320C67X_REG_A0) && (op->RegVal <= TMS320C67X_REG_A31))
					op->RegVal = (op->RegVal - TMS320C67X_REG_A0 + TMS320C67X_REG_B0);
...
{
	MCRegisterInfo_InitMCRegisterInfo(MRI, TMS320C67xRegDesc, 90,
			0, 0,
			TMS320C67xMCRegisterClasses, 7,
			0, 0,
			TMS320C67xRegDiffLists,
			0,
			TMS320C67xSubRegIdxLists, 1,
			0);
}
